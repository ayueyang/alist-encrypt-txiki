...
// 初始化目录
if (!(await fs.exists(getConfPath()))) {
...
const exist = await fs.exists(getConfFilePath())
if (!exist) {
  // 把默认数据写入到config.json
  const configData = { alistServer: alistServerTemp, webdavServer: webdavServerTemp, port: 5344 }
  await fs.writeFile(getConfFilePath(), JSON.stringify(configData, '', '\t'))
}
// 读取配置文件
const configJson = await fs.readFile(getConfFilePath(), 'utf8')
const configData = JSON.parse(configJson)

export function normalizeAlistConfig(alistServerConfig) {
  if (!Array.isArray(alistServerConfig?.passwdList)) {
    return alistServerConfig
  }
  for (const passwdData of alistServerConfig.passwdList) {
    if (Array.isArray(passwdData.encPath)) {
      passwdData.encPath = passwdData.encPath.map((filePath) => (typeof filePath === 'string' ? filePath.replace(/\/{2,}/g, '/') : filePath))
    }
  }
  return alistServerConfig
}

normalizeAlistConfig(configData.alistServer)
...
if (configData.alistServer.flowPassword) {
  ...
  await fs.writeFile(process.cwd() + '/config.json', JSON.stringify(configData, '', '\t'))
}
