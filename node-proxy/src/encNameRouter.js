...
function normalizeRawUrl(rawUrl, serverAddr) {
  try {
    const rawUrlInfo = new URL(rawUrl)
    const serverUrlInfo = new URL(serverAddr)
    if (rawUrlInfo.hostname === serverUrlInfo.hostname && !rawUrlInfo.port && serverUrlInfo.port) {
      rawUrlInfo.port = serverUrlInfo.port
      return rawUrlInfo.toString()
    }
  } catch {}
  return rawUrl
}
...
encNameRouter.all('/api/fs/get', bodyparserMw, preHandleFolderPath, async (ctx, next) => {
  ...
  if (passwdInfo) {
    ...
    const rawUrl = normalizeRawUrl(result.data.raw_url, ctx.req.serverAddr)
    await levelDB.setExpire(key, { redirectUrl: rawUrl, passwdInfo, fileSize: result.data.size }, 60 * 60 * 72) // 缓存起来，默认3天，足够下载和观看了
