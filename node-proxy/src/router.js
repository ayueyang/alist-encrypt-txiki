...
import fs from 'fs'
import { alistServer, webdavServer, port, initAlistConfig, normalizeAlistConfig, version } from './config'
...
router.all('/saveAlistConfig', async (ctx, next) => {
  ...
  normalizeAlistConfig(alistConfig)
  const _snapshot = JSON.parse(JSON.stringify(alistConfig))
  // 写入到文件中，这里并不是真正的同步，，
  await fs.writeFile(process.cwd() + '/config.json', JSON.stringify({ alistServer: _snapshot, webdavServer, port }, '', '\t'))
...
router.all('/saveWebdavConfig', async (ctx, next) => {
  ...
  await fs.writeFile(process.cwd() + '/config.json', JSON.stringify({ alistServer: alistServer._snapshot, webdavServer, port }, '', '\t'))
  ctx.body = { data: webdavServer }
})
...
router.all('/encryptFile', async (ctx, next) => {
  ...
  const files = await searchFile(folderPath)
