...
output_dir = Path(__file__).resolve().parent.parent / 'output' / 'playwright'
...
with sync_playwright() as playwright:
    ...
    print('BROWSER_INSPECT', {'url': page.url, 'title': page.title(), 'inputs': page.locator('input').count(), 'buttons': page.locator('button').count()})
    ...
    print('BROWSER_BODY', page.locator('body').inner_text()[:1000].replace('\n', ' | '))
    ...
    print('BROWSER_CONFIG', {'url': page.url, 'title': page.title(), 'body': page.locator('body').inner_text()[:1200].replace('\n', ' | ')})
    ...
    if '@login' in page.url:
        ...
        if '@login' in page.url:
            raise RuntimeError(f'AList browser login did not complete: {page.url}')
        page.goto(f'{origin}/', wait_until='networkidle')
        page.wait_for_timeout(1000)
    page.screenshot(path=str(output_dir / 'proxy-home.png'), full_page=True)
    print('BROWSER_PROXY', {'url': page.url, 'title': page.title(), 'body': page.locator('body').inner_text()[:1000].replace('\n', ' | ')})
    print('BROWSER_ERRORS', {'console': console_errors, 'page': page_errors, 'requests': request_failures})
    browser.close()
